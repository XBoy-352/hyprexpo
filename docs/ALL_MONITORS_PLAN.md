...
|---|---|---|
| **Foreign monitor unplugged while grid is open.** Fork has **zero** monitor-disconnect listeners; the `onCursorMove` weak-lock guard (Overview.cpp:903-905) only protects the **overview's own** monitor. On unplug Hyprland reassigns `workspace->m_monitor` to a survivor; a cached cell then renders a migrated workspace and a path-(a) click `focusMonitor`s the **wrong** survivor. | **critical** | Add a monitor-added/removed listener for `g_pOverview`'s lifetime that **closes the overview** (cheapest, safest) — or forces a full `redrawAll` + cell re-resolution. Never trust a cell's cached home monitor across frames. Combine with §3b (re-resolve by ID) and §4 (TOCTOU re-validate `HOME`). |
